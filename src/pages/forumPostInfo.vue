<template>
  <div>
    <div class="post">
      ...
      <div class="postContent">
        ...
        <div class="createTime">
          发表于：
          {{post.createTime | formatLongDate("yyyy-MM-dd HH:mm")}}
        </div>
        <div class="btnArea">
          <div v-if="containsUser(post.forum.managers, getLoggedInUser())" id="showDetail"
    ...
    <div v-for="reply in post.forumPostReplies" class="post">
      <div class="userInfo">
      ...
      <div class="postContent">
        ...
        <div class="createTime">
          发表于：
          {{reply.createTime | formatLongDate("yyyy-MM-dd HH:mm")}}
        </div>
        <div class="btnArea">
          <div v-if="containsUser(post.forum.managers, getLoggedInUser())" id="showDetail"
    ...
    <div class="replyArea">
      ...
      <div style="display:flex; justify-content: center">
        ...
        <div style="float: left; width: 8em; margin-left: 0.5em;">
          <x-button type="primary" @click.native="$router.go(-1)">返回论坛</x-button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPostInfo () {
        api.getForumPostById(this.$route.query.postId).then((res) => {
          this.post = res.data
        })
      },
      saveReply () {
        ...
        api.replyForumPost(this.post.id, this.replyContent).then((res) => {
          if (res.data.success) {
            this.info('回复成功')
            this.replyContent = ''
            this.getPostInfo()
